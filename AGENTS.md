...
### Dealer route mapping + order route/salesman snapshot (as of 2026-06-06)

Context: in Focus, a dealer (`vmCore_Account`, `iAccountType=5`, `D…` codes) is mapped to a **route** via its `SalesmanName` column (e.g. `R-A`), not directly to a salesman. The `Core__salesman` master carries **both** the route (`sName`, R-A) and the **actual salesman** (`SalesmanName`, S-A) plus the salesman phone (`SalesManPhNO`). Many routes share one salesman/phone, and `mobile` is globally unique in our `User` model — so multiple routes cannot each be a distinct user. We therefore store the route on the dealer for display and keep `salesExecutive` as the (shared) salesman mobile.

- `models/User.js` — added `routeName: { type: String }` on the dealer = the Focus route (`vmCore_Account.SalesmanName`, R-A). Clarified `salesExecutive` = the actual salesman's (S-A) mobile, shared across that salesman's routes.
- `models/Order.js` — added an order-time **snapshot** so history doesn't shift when a dealer is re-mapped later:
  - `routeName` (display) and `salesExecutiveMobile` (S-A mobile) — flat.
  - `focusRefs` (typed sub-schema, `_id:false`) groups the Focus master IDs actually posted: `customerAccountId` (CustomerAC__Id), `salesmanId` (Salesman__Id / route iMasterId), `districtsId` (Districts__Id), `isIGST`. User-selected context (`entityId/warehouseId/branchId/narration`) stays flat because it's queried/filtered. Footer is always `[]`; Body = `items` (already stored) — nothing extra persisted.
- `scripts/update-dealer-salesexec.js` — `salesmanMap` now carries `routeName` (sName) **and** `salesmanName` (SalesmanName, falling back to route name if blank). Auto-created `SalesExecutive` accounts are named after the actual salesman (deterministic, no longer dependent on Focus row order). Also **corrects existing** `SalesExecutive` users: matches by mobile against `phoneToSalesmanName` (first salesman name seen per phone) and `$set`s `name` to the salesman (S-A) only when it differs — fixing accounts previously named after the route `sName`; reported as `Existing Sales Executive names corrected: N`. Dealer updates now set `routeName` (R-A) and **always-overwrite** `primaryContactPerson` (now the salesman name S-A) and `primaryContactPersonMobile` (S-A phone), instead of the previous set-once behavior; `salesExecutive` (S-A phone) is still always synced.
- `services/focus8Order.service.js` — `getOrderHeaderData` returns `SalesmanName` (route); `pushOrderToFocus8` returns `routeName` + `customerAccountId`/`salesmanId`/`districtsId`/`isIGST`.
- `controllers/ordersController.js` — `createOrder` snapshots `routeName` + `salesExecutiveMobile` from the dealer at creation; `runFocusSync` writes the authoritative `routeName` + grouped `focusRefs` from Focus on sync success. `getOrderDetails` now displays the **route in place of "sales executive"** (snapshot-first, dealer-mapping fallback), removing the old fragile `dealer → phone → SalesExecutive.name` lookup. Populates include `routeName`.
- `controllers/userController.js` — `routeName` added to `USER_ALLOWED_CREATE_FIELDS` (and therefore `USER_ALLOWED_UPDATE_FIELDS`), making it admin read/create/update-able. Safe because order-time route/IDs are taken fresh from Focus and frozen on the order; the dealer's `routeName` is display/convenience only. Note: the sync script still always-overwrites `routeName`, so manual edits persist only until the next run. Also added `getRoutes` handler (10-min in-memory cache) backing `GET /users/routes` (admin) — returns the canonical Focus route list `[{ routeName, salesmanName, salesExecutiveMobile }]` so the admin UI offers a route **dropdown** (no free-text — `routeName` is a join key; a typo makes Focus resolve `Salesman__Id` to 0). Sourced from `getRouteMaster()` in `services/focus8Order.service.js` — returns **one entry per `Core__salesman` row, no dedup** (the same route name can belong to different salesmen; the salesman name is returned so the UI can disambiguate), normalizing `SalesManPhNO` to the last-10-digit salesman mobile.
- `routes/usersRoutes.js` — added `GET /routes` (`requireRole(ADMIN)`) → `userController.getRoutes`.
- `mongoscripts/backfill_order_route_snapshot.js` *(new)* — one-shot, idempotent backfill that freezes `routeName` + `salesExecutiveMobile` onto legacy orders (which predate the snapshot) from the dealer's current mapping (`dealerId`, else `createdBy` when a Dealer — mirrors `getOrderDetails`). Only fills empty fields (never clobbers a Focus-sourced `routeName`); skips `focusRefs` (not reconstructable, no display use). Run `scripts/update-dealer-salesexec.js` FIRST so dealers have `routeName`. Note: it freezes the *current* mapping, not the true point-in-time route.
